...
public class ReportController {
    ...
    @FXML
    private void handleFilterByMonthYear() {
        ...
        // Cập nhật DatePicker (để người dùng thấy)
    ...
    private void loadKpiCards(LocalDate start, LocalDate end) throws SQLException {
        ReportStats stats = hoaDonDAO.getReportStats(start, end);
    ...
    private void loadRevenueChart(LocalDate start, LocalDate end) throws SQLException {
        List<DailyStats> dailyData = hoaDonDAO.getDailyRevenue(start, end);
        revenueBarChart.getData().clear();
        
        XYChart.Series<String, Number> series = new XYChart.Series<>();
        series.setName("Doanh thu");
        
        for (DailyStats ds : dailyData) {
            series.getData().add(new XYChart.Data<>(ds.getNgay().toString(), ds.getDoanhThu()));
        }
        revenueBarChart.getData().add(series);
    }
    
    private void loadGenreChart(LocalDate start, LocalDate end) throws SQLException {
        List<GenreStats> genreData = theLoaiDAO.getGenreStats(start, end);
        genrePieChart.getData().clear();
        
        for (GenreStats gs : genreData) {
    ...
    private void runReport(LocalDate start, LocalDate end) {
        try {
